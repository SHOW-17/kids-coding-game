# 🦄 あそびのもり

小学校1年生（6〜7歳）が、**文字を読まずに**遊びながら賢くなれる、こども向けのミニゲーム集（アーケード）。

オープニング画面（`index.html`）でゲームを選び、それぞれの遊びに入る。すべて **指タップ前提・ひらがな＋アイコンのみ・ビルド不要**（バニラJSの単一HTML）。

> **デザインテーマ「ねんどの森」** — ねんど（クレイ）／フェルトの質感の世界。マスコットはユニコーン（ブルー＝主役／グリーン＝はかせ／パープル＝ごほうび）。アイコンは絵文字でなく CSS図形、キャラは `assets/uni/` の画像で表現する。視覚言語の定義は [Design.md](Design.md) を参照（新画面はこれに従う）。

- **対象**：小1（漢字を読めない前提）。UIはひらがな＋アイコンのみ。
- **ねらい**：知的好奇心をくすぐり、非認知能力（集中・やりぬく力・試行錯誤・空間認識・創造性）を伸ばす。
- **やさしさ**：失敗しても叱らず、何度でも試せる。クリアは効果音・アニメ・紙吹雪で「できた！」を強く演出。
- **じゆうに あそべる**：どのゲームも好きなレベル／むずかしさを選んで遊べる（後述）。
- **品質**：プロ品質を目標に、見た目・演出・手触りまで作り込む。

---

## あそべるゲーム

| ゲーム | 仕組み | 育てる力 |
|---|---|---|
| **プログラミング** | やじるしを並べて▶。命令どおりにユニコーンが動く | シーケンス・デバッグ・論理 |
| **まねっこ**（サイモン） | 光った順番を覚えてまねする | 記憶・集中・やりぬく力 |
| **きまり あそび** | 並びの「つぎは なに？」を当てる | 規則性の発見・推理 |
| **かたち づくり** | ピースを影にはめて形を作る | 空間認識・創造性・試行錯誤 |
| **ぴたごら** | 坂を置いて玉をゴールへ転がす | 因果・計画・試行錯誤 |
| **とまれ あそび** | ユニコーンはタップ、とげとげは がまん（Go/No-Go） | 抑制制御・がまん力・集中 |
| **どこちがう** | 2枚の絵から ちがいを探す（自動生成） | 観察力・注意力 |
| **そっくりわけ** | いろ・かたち・おおきさで かごに分ける。途中でルールが切り替わる | 分類・認知的柔軟性（きりかえ力） |
| **ぺあさがし**（神経衰弱） | カードをめくって同じ絵のペアを探す | 視覚空間記憶 |
| **うたあそび** | 音の高さを聞き分け、マイクに歌って音程を合わせる | 音感（聴覚弁別＋発声フィードバック） |

「プログラミング」のレベル構成や設計詳細は [CLAUDE.md](CLAUDE.md) を参照。

#### プログラミングのブロック操作（ドラッグ＆ドロップ対応）

- **タップ**：パレットのやじるしを押すと末尾（くりかえしを ひらいて いれば その中）に追加。従来どおり。
- **ドラッグ**：チップを指でつまんで **ならびかえ**／**くりかえし枠への出し入れ**／**パレットから好きな位置へ挿入**。挿入位置はオレンジの点滅バーで表示。
- **ゴミ箱で削除**：ドラッグ中だけ画面下に出るゴミ箱に落とすと、その1個だけ削除（ポフッと消える）。途中のブロックも直せる＝デバッグしやすい。
- **くりかえしの回数は −/＋ボタン**（2〜8回）。枠をタップすると「ひらく/とじる」が切り替わり、ひらいている間は点滅カーソルで「次はここに入る」が見える。

### ごほうび — わたしの おへや（`room.html`）

どのゲームでも遊ぶと **どんぐり**（横断通貨）がたまり、ホームの「わたしの おへや」入口から
**どんぐりくじ**を引ける。当たると、自分のユニコーンの **きせかえ**（ぼうし・めがね・くび）や
**おへやの かぐ** がもらえる ＝ 全ゲームを横断して遊ぶ継続動機になる。

- **どんぐりの数え方**：**クリア／せいかいするたびに もらえる**（再クリアでも増える）。さらに**むずかしいステージほど多くもらえる**（ぴたごら・かたち・プログラミングは5レベルごと、まねっこは深い段ほど、きまりは問が進むほど）。各ゲームが「せいこう量（`wins`）」を貯め、その合計がどんぐり。減らない＝失敗しても損しない。これまで貯めた分は移行ボーナスで引き継ぐ。
- **どんぐりゲット演出**：クリアした瞬間に「🌰＋N」が噴き上がる即フィードバック。難しいステージほど N が大きい。各ゲームは成功のたびに `FX.acornGain(N)` を出すため `rewards.js` を読み込む（programming だけは自己完結のため独自の `acornPop()` で同じ演出）。
- **くじ**：1回どんぐり4こ。まだ持っていないごほうびから当たる（ダブりなし）。着せ替えは自動で装備、かぐは部屋に並ぶ。全部そろうと「ぜんぶ あつめた！」。
- **通知**：くじが引けるとホームの入口に「！」バッジが出る。
- ごほうび・おへや・ガチャの見た目は **画像アセット**（`assets/items/` `assets/room/` `assets/gacha/`。ユニコーンと同じつやつやクレイ調）。きせかえはユニコーンに透過PNGを重ねて表示、かぐは部屋に並ぶ。
- **くじ演出**：どんぐりガチャマシンが回る → 虹色のたまごが出てくる → タップで割れてごほうびが飛び出す＋紙吹雪＋効果音。文字を読まなくても「もらえた！」が伝わる。
- **じゆうに かざる（ドラッグ配置）**：きせかえも かぐも、指でつかんで好きな位置に動かせる。置いた場所は保存され、次に開いても・着せ直しても同じ位置に戻る（`room` 名前空間の `pos`）。

### レベル／むずかしさを じゆうに えらべる

- **レベルのあるゲーム**（プログラミング・かたちづくり・ぴたごら）：ステータスバーの **◀ ▶** で前後のレベルへ自由に移動できる（ロックなし＝1〜最終レベルまでどこへでも）。進捗の **★／●ドットを直接タップ**しても飛べる。選んだレベルは保存され、次に開いたときも続きから。
- **レベルのないゲーム**（まねっこ・きまり）：個別レベルが無く難度が自動で上がる仕組みのため、代わりに **「むずかしさ えらび」** を用意。まねっこ＝光る速さ（ゆっくり／ふつう／はやい）、きまり＝出題の難しさ（やさしい／ふつう／むずかしい）。選択は保存される。

### 最高記録はホームで確認

アーケード（`index.html`）の各ゲームカードに、その子の **最高記録** を表示する。

| ゲーム | ホームに出る記録 |
|---|---|
| プログラミング | クリア数バー（◯/19）＋「さいこう レベルN」（到達した最高レベル） |
| まねっこ | 「さいこう Nだん」 |
| きまり | 「れんぞく Nもん」（連続正解の最高） |
| かたちづくり | 「Nこ かんせい」 |
| ぴたごら | 「Nこ クリア」 |
| とまれあそび | 「さいこう Nてん」 |
| どこちがう | 「Nもん クリア」 |
| そっくりわけ | 「Nこ せいかい」 |
| ぺあさがし | 「Nかい クリア」 |
| うたあそび | 「Nかい クリア」 |

---

## 起動方法

### 公開サイト（GitHub Pages・いちばん簡単）

**https://show-17.github.io/kids-coding-game/** にアクセスするだけで遊べる。
iPhone/iPad は Safari で開いて「ホーム画面に追加」、Android は Chrome の「アプリをインストール」で
PWA としてインストールでき、2回目以降はオフラインでも動く（Service Worker が全ファイルをキャッシュ）。
master ブランチに push すると自動で反映される。

### ローカルで動かす

ビルド不要。次のいずれかで `index.html` を開く。

```bash
# そのままブラウザで開く
open index.html            # mac
xdg-open index.html        # linux

# 簡易サーバーで配信（スマホ実機・Tailscale 等から見る場合）
python3 -m http.server 9090 --bind 0.0.0.0
# → http://<サーバーのIP>:9090/
```

`index.html` がアーケード（オープニング）。各ゲームは `games/*.html`。

### Androidタブレットで遊ぶ（オフライン）

2つの方法がある。どちらも**ネット接続なしで遊べる**（フォントは `assets/fonts/` に同梱済み・外部CDN依存なし）。

**① Androidアプリ（APK・おすすめ）** — Capacitor で Web 資産を丸ごと同梱した完全オフラインアプリ。
INTERNET 権限なし＝通信は一切しない。ストア不要、APK をタブレットに送って「提供元不明のアプリ」を許可してインストール（サイドロード）。

```bash
# ビルド手順（要：JDK 21・Android SDK。署名鍵は ~/.android-keys/ に別管理）
node scripts/build-sw.mjs       # sw.js（プリキャッシュ一覧）を再生成
node scripts/build-www.mjs      # www/ に公開ファイルを集める
npx cap sync android            # www/ を android プロジェクトへ同期
cd android && ./gradlew assembleRelease
# → android/app/build/outputs/apk/release/app-release.apk
```

**② PWA（ブラウザにインストール）** — HTTPSで配信した `index.html` を Chrome で開き「ホーム画面に追加」。
Service Worker（`sw.js`）が全ファイルをキャッシュし、2回目以降はオフラインで動く。
アセットを追加・変更したら `node scripts/build-sw.mjs` で `sw.js` を作り直すこと（キャッシュ版数が変わる）。

---

## ファイル構成

```
index.html              アーケード選択画面（オープニング／PWAのホーム）
room.html               わたしの おへや（ごほうび＝どんぐりくじ・きせかえ・かぐ）
Design.md               デザインシステム「ねんどの森」の定義（視覚言語の本体）
manifest.webmanifest    PWAマニフェスト
games/
  programming.html      プログラミング（🏠でアーケードへ戻る。assets JS 不要の自己完結）
  manekko.html          まねっこ（サイモン）
  kimari.html           きまりあそび（パターン推理）
  katachi.html          かたちづくり（タングラム＝かげあわせ方式）
  pitagora.html         ぴたごら（たまころがし＝決定論グリッド）
  tomare.html           とまれあそび（Go/No-Go＝がまん力）
  chigai.html           どこちがう（まちがいさがし＝自動生成＋検証）
  sokkuri.html          そっくりわけ（分類＋ルール切り替え）
  pair.html             ぺあさがし（神経衰弱）
  uta.html              うたあそび（音程トレーニング。マイク使用）
assets/
  menu/                メニュー画面（index.html）の画像素材一式（WebP）。
                       背景 bg / ロゴ logo / ユニコーン uni_* / ボタン btn_* /
                       ゲーム看板 banner_* / カードアート card_*
  uni/                  ユニコーン画像（blue_/green_/purple_*。ゲーム内マスコット）
  bg/                   各ゲームのシーン背景（webp。空＋下1/3の丘を描いた縦長シーン画像）
  tokens/               盤面トークン画像（webp 透過。ほし・はこ・まと・ボタン・とびら・
                        うずまき・たま・かご・とげボール）
  pairs/                ぺあさがしのカード絵柄12種（webp 透過。りんご・ほし・はな・ちょう 等）
  icons/               ファビコン・PWA・apple-touch アイコン一式
  fonts/                セルフホストフォント（fonts.css + woff2。オフライン対応）
  audio.js              共通サウンド（効果音。Web Audio）
  fx.js                 共通エフェクト（紙吹雪・キラキラ・シェイク）
  save.js               共通セーブ（ゲーム別 localStorage）
  rewards.js            ごほうび管理（どんぐり集計・くじ抽選・所持/装備。index と room で共有）
  shell.js              共通UI（ヘッダー・トースト・モーダル・画面遷移）
  bgm.js                共通BGM（画面ごとにループ再生。mp3優先・無ければ合成）
  bgm/                  BGM音源置き場（mp3。AI生成曲をコミット済み。権利クリアな音源のみ可）
sw.js                   Service Worker（自動生成。直接編集しない）
capacitor.config.json   Capacitor 設定（Androidアプリ化）
android/                Android ネイティブプロジェクト（Capacitor 生成・Gradle）
scripts/
  test-games.js         ヘッドレス・スモークテスト
  build-sw.mjs          sw.js 生成（プリキャッシュ一覧の更新）
  build-www.mjs         Capacitor 用 www/ の組み立て
```

> スタイルは各HTMLに**インライン**で持つ（共通の見た目ルールは [Design.md](Design.md)）。
> ※ プログラミングの保存キー `kuma_prog_save_v1` はJS内の定数（ファイル名・パスに依存しない）。
>   旧 `kuma-programming.html` から `games/programming.html` へ移動・改名したが、保存データは
>   localStorage がオリジン単位のため引き継がれる。

---

## 技術メモ

- バニラJS／ビルド不要。共通基盤（`assets/*`）の上に各ゲームを単一HTMLで実装。
- フォント：Mochiy Pop One / M PLUS Rounded 1c（Google Fonts 由来・OFL）。**woff2 を `assets/fonts/` にセルフホスト**しオフラインでも崩れない（外部CDNへの参照なし）。
- 効果音：Web Audio で生成（音声ファイル不要、try/catch でガード）。
- 画面の四隅UIは全画面で統一：**左上にホーム/せってい、右上にBGMトグル**（いずれも `position:fixed` でスクロール追従）。
  各ゲーム画面は左上に「おうちへ もどる」ホームボタンを固定（メニューへ戻る）。
- **レスポンシブ（スマホ／タブレット／横画面に対応。とくにタブレット横を重視）**：
  - 縦画面は従来どおり中央1カラム。**横画面（`min-width:720px` かつ landscape）では2カラムに自動で組み替え**、
    左に盤面／プレイ領域、右にレベルバー・命令・道具・操作をまとめる（CSS Grid／メディアクエリ。HTMLは縦画面と共通）。
  - 盤面・ピース・ダイヤル等のサイズは**画面の幅だけでなく高さも見て**算出し、横画面でも縦スクロールせず1画面に収まる
    （`programming` の `cellSize`、`katachi`/`pitagora` の盤面・トレイ計算などを向き対応に変更）。
  - メニュー（`index`）とおへや（`room`）も横画面で横幅を活かし、カード／ずかんを多カラム化してスクロールを削減。
- BGM：画面ごとに割り当ててループ再生（`assets/bgm.js`）。右上の音符ボタンで ON/OFF（全画面共通で保存）。
  `assets/bgm/<画面名>.mp3` を置けばその曲を優先再生、無ければやさしい合成ループを暫定で鳴らす。
  現在は AI生成の `_default.mp3` をコミット済み。**公開リポジトリのため権利クリアな音源のみ**置けること
  （市販曲は不可）。詳細は [`assets/bgm/README.md`](assets/bgm/README.md)。
  音量は効果音の邪魔をしない控えめ設定。`Bgm.stop()` で明示的に止めた画面では、タップしても
  BGM が勝手に再開しない（うたあそびのマイク中に鳴り出す不具合を修正済み）。
- 永続化：`localStorage`（ゲーム別の名前空間。使えない環境ではメモリに自動フォールバック）。
  メニュー左上の「せってい」（歯車）→「きろくを ぜんぶ けす」で全消去（子どもが誤操作しないよう設定内に格納）。
- アクセシビリティ：`prefers-reduced-motion` を尊重。色だけに頼らない表現。
- 動作確認：`google-chrome-stable` + puppeteer-core で各ページの console error / 例外 / 横スクロールはみ出しを検査（`scripts/test-games.js`。実行手順はファイル冒頭のコメント参照）。
- **うたあそびのドレミ表示と「うた声」お手本**：お手本の音は ピッ音でなく、フォルマント合成
  （のこぎり波＋「お」の口の形の bandpass＋ビブラート）で「どー」と歌っている風の声（`Voice.sing`）。
  まねうたは 階段に音名（ど・れ・み・そ・ら・ど）を表示し、いま出す音を吹き出しで大きく見せる
  （「『どー』って うたってね！」）。こえでとばそ のリングにも音名を表示＝どの高さの声か一目で分かる。
- **マイク（うたあそびのみ）**：`getUserMedia` ＋ Web Audio の自己相関でリアルタイム音程検出。
  ブラウザは HTTPS（または localhost）でのみマイクが使える。Android アプリは `RECORD_AUDIO` 権限を宣言
  （INTERNET 権限は引き続きなし＝音声は端末外に出ない）。マイク不可の環境でも「どっちが たかい？」
  （聞き分けモード）は遊べる。

### デザイン素材について
メニュー画面（`index.html`）は、ロゴ・背景・ユニコーン・ゲーム看板・カードアート・ボタンを
**画像素材（`assets/menu/*.webp`）** で構成し、チープ感を排した作り込みにしている。
各ゲーム画面（`games/*.html`）も、**シーン背景（`assets/bg/*.webp`。ゲームのテーマ色で描いた
クレイ調の空と丘）** と **盤面トークン画像（`assets/tokens/*.webp`。ほし・はこ・とびら・たま等）**
で構成し、メニューと同じ画像クオリティに統一。UIアイコン類は引き続き CSS/SVG で「ねんど化」
（**絵文字は使わない**）。
画像素材は LLM 画像生成（ChatGPT / Claude Design 等）で用意し、白背景は切り抜き → WebP 最適化して配置する運用。
マスコットのユニコーンは [Design.md](Design.md) の配役ルール（ブルー＝主役／グリーン＝はかせ／パープル＝ごほうび）に従う。

---

## ライセンス

Private. 個人開発・学習用。
