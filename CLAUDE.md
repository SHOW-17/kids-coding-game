...
「くまさんプログラミング」。以降、知育になるゲームを増やしていく。
...
kuma-programming.html    くまさんプログラミング（第1弾。🏠でindexへ戻る）
games/
...
assets/
  arcade.css            共通デザインシステム（トークン・ボタン・カード・モーダル・アニメ）
...
| ゲーム | 仕組み | 育てる力 | アクセント色 |
|---|---|---|---|
| くまさんプログラミング | 命令を並べて動かす | シーケンス・デバッグ・論理 | 橙(はちみつ) |
| まねっこ（サイモン） | 光った順番を記憶して再現 | 記憶・集中・やりぬく力 | 紫 |
| きまりあそび | 並びの「つぎ」を当てる | 規則性発見・推理・論理 | 青 |
| かたちづくり | ピースを影にはめる（回転なし） | 空間認識・創造性・試行錯誤 | 緑 |
| ぴたごら | 坂を置いて玉をゴールへ | 因果・計画・試行錯誤 | 橙 |
